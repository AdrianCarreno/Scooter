...
class I2C:
    ...
    def callback(self, data):
        try:
            self.BUS.write_byte(self.ADDR_THROTTLE, self.data.throttle)
        except IOError:
            rospy.logerr("Can't send data to I2C device address %s", "0x{:02x}".format(self.ADDR_THROTTLE))
        try:
            self.BUS.write_byte(self.ADDR_DIR, self.data.direction)
        except IOError:
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            try:
                ...
                self.data.dir = self.translate(direction, -127, 127, -135, 135)
